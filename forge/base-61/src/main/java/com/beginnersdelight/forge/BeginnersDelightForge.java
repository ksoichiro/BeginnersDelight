...
public class BeginnersDelightForge {
    public BeginnersDelightForge(FMLJavaModLoadingContext context) {
        BeginnersDelight.init();
        ForgeGameRules.register(context.getModBusGroup());
        VillageConfigLoader.load(FMLPaths.CONFIGDIR.get());
        if (FMLEnvironment.dist == Dist.CLIENT) BeginnersDelightForgeClient.init();
    }
    ...
    public static class ForgeEvents {
        ...
        @SubscribeEvent public static void onPlayerJoin(PlayerEvent.PlayerLoggedInEvent event) {
            if (event.getEntity() instanceof ServerPlayer player) {
                ...
                StarterHouseGenerator.onPlayerJoin(player);
            }
        }
        @SubscribeEvent public static void onPlayerRespawn(PlayerEvent.PlayerRespawnEvent event) {
            if (event.getEntity() instanceof ServerPlayer player) {
                ...
                StarterHouseGenerator.onPlayerRespawn(player, event.isEndConquered());
            }
        }
        @SubscribeEvent public static void onServerTick(TickEvent.ServerTickEvent.Post event) {

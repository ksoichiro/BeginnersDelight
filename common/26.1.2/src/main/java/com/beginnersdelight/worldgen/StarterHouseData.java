...
import com.beginnersdelight.util.SavedDataMigration;
...
public class StarterHouseData extends SavedData {
    ...
    // MC 26.1+ identifies SavedData by Identifier (stored at data/<namespace>/<path>.dat).
    // DATA_NAME is kept for migrating the flat data/<name>.dat file from older versions.
    public static final SavedDataType<StarterHouseData> TYPE = new SavedDataType<>(
            Identifier.fromNamespaceAndPath(BeginnersDelight.MOD_ID, "starter_house"),
    ...
    public static StarterHouseData get(ServerLevel level) {
        SavedDataMigration.migrateLegacyFile(level, DATA_NAME, TYPE);

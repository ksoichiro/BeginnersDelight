#!/usr/bin/env python3
"""One-off propagation of the jungle-tree-clearing shrink fix to all
version-specific VillageHouseGenerator.java / StarterHouseGenerator.java
copies. 26.2 was hand-edited and verified; this replicates the same three
anchor-based edits to the other 18 version directories.
"""
import sys
...
CONST_ANCHOR = "    private static final int MAX_LEAF_DISTANCE = 7;\n"
CONST_REPLACEMENT = CONST_ANCHOR + """
    // Jungle trees are unusually large (megatrees, 2x2 trunks, canopy far from the
...
CLASSIFY_ANCHOR = """                if (isInsideClearedArea(x, z, minX, maxX, minZ, maxZ)) {
...
"""
CLASSIFY_REPLACEMENT = """                boolean inside = isJungleLog(level.getBlockState(trunkBase))
...
EDITS = [
    ...
    (CLASSIFY_ANCHOR, CLASSIFY_REPLACEMENT),
